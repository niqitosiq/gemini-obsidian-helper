...
def schedule_task(task_details: dict, source: str) -> tuple[bool, str]:
    ...
    language = "ru" if any(char in action for char in "абвгдеёжзийклмнопрстуфхцчшщъыьэюя") else "en"
    ...
    logger.debug(f"Task details - Action: {action}, Duration: {duration_minutes} min, Deadline: {deadline_str}")

    if not action:
        msg = "Не удалось определить суть задачи. Попробуйте переформулировать." if language == "ru" else "Could not extract task essence. Try rephrasing."
    ...
    start_time = now + timedelta(minutes=15 - now.minute % 15)  # Round to next 15 minutes
    duration = timedelta(minutes=duration_minutes if duration_minutes is not None else DEFAULT_DURATION_MINUTES)
    ...
    if deadline_str:
        try:
            ...
            if target_time.date() > now.date():
                target_time = datetime.combine(target_time.date(), time(config.WORK_START_HOUR))
            
            # Use target time as the start time
            start_time = target_time
            schedule_in_slot = True
        except ValueError:
            logger.warning(f"Invalid deadline format: {deadline_str}, using current time")
    ...
    if schedule_in_slot:
        ...
        for task in existing_tasks_raw:
            if task.due and task.due.datetime:
                try:
                    task_start = datetime.fromisoformat(task.due.datetime.replace("Z", "+00:00")).replace(tzinfo=None)
        ...
        if found_slot:
            ...
            logger.info(f"Scheduler: Found slot for '{action}' at {due_string} (duration: {duration_minutes} min)")
        else:
    ...
    # Add duration to task description
    if duration_minutes:
        details = f"Продолжительность: {duration_minutes} минут\n{details}" if language == "ru" else f"Duration: {duration_minutes} minutes\n{details}"
    ...
    return bool(created_task), _format_response_message(created_task, due_string, project_id, priority, language)
